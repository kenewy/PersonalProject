...
if response.status_code == 200:
    ...
    temperature_max_arr = [round(temp) if not pd.isna(
        temp) else "-" for temp in temperature_max]
    temperature_min_arr = [round(temp) if not pd.isna(
    ...
    moyenne = []
    for i in range(4):
        moyenne.append((temperature_max_arr[i]+temperature_min_arr[i])/2)
else:
...
noms_urls = []
for i in wmo:
    if str(i) in correspondance_wmo:
...
Ces descriptions sont affichées en survolant les cellules avec le curseur. Toutes les données affichent "-" si elles sont indisponibles.
