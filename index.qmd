...
if response.status_code == 200:
    ...
    # Prendre les valeurs entières
    temperature_max_ent = [round(temp) for temp in temperature_max]
    temperature_min_ent = [round(temp) for temp in temperature_min]
    ...
    # Remplacer les valeurs manquantes par la valeur 100 (qui affichera une image d'erreur) pour le code wmo
    wmo = [code_wmo if not pd.isna(
        code_wmo) else 100 for code_wmo in wmo]
...
if response_h.status_code == 200:
    ...
    # Remplacer les valeurs manquantes par la valeur 100 (qui affichera une image d'erreur) pour le code wmo_h
    wmo_h = [code_wmo_h if not pd.isna(
        code_wmo_h) else 100 for code_wmo_h in wmo_h]
else:
    # Affichage d'un message d'erreur si la requête échoue
...
# Afficher la température et la couleur associée si possible
...
for i in range(jours+1):  # Chaque jour
    ...
    if a != 0:  # Si on a au moins une donnée
        # Moyenne du vent du jour i selon les valeurs disponibles
...
Certaines descriptions sont affichées en survolant les cellules et les images avec un curseur de souris. Il peut arriver que les symboles "-" ou "/" s'affichent à la place de certaines données indisponibles.
